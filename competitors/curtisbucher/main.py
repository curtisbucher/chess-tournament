...
def get_move(last_move):
    ...
    if not last_move:
        ...
        # If this engine is white, converting input from black to white
        if side:
            start = chr(201 - ord(last_move[0])) + str(9 - int(last_move[1]))
            end = chr(201 - ord(last_move[2])) + str(9 - int(last_move[3]))
            last_move = start + end

        # We query the user until she enters a (pseudo) legal move.
        match = re.match('([a-h][1-8])'*2, last_move)
    ...
    if not side:  # If this engine is black, convert from white to black
        return render(119 - move[0]) + render(119 - move[1]) + promotion * "q"
    return render(move[0]) + render(move[1]) + promotion * "q"


if __name__ == '__main__':
    while True:
        print(get_move(input("Move: ")))
